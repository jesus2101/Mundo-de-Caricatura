  ...
  class Usuario extends Conexion
  {
    ...
    public function login($Nombre_Usuario, $contrasena)
    {
      ...
      */
      ...
      $contrasena = parent::salvar($contrasena);
      ...
      */
      ...
      $consulta = 'select id_usuario, from usuarios where email="'.$user.'" and clave= MD5("'.$clave.'")';
      /*
    ...
    public function registroUsuario($nombre, $A_paterno, $Nombre_Usuario,$correo,$contrasena,$confirma_contrasena)
    {
      parent::conectar();

      $nombre  = parent::filtrar($nombre);
      $A_paterno = parent::filtrar($A_paterno);
      $Nombre_Usuario = parent::filtrar($Nombre_Usuario);
      $correo = parent::filtrar($correo);
      $contrasena = parent::filtrar($contrasena);
      $confirma_contrasena = parent::filtrar($confirma_contrasena);



      // validar que el correo no exito
      $verificarCorreo = parent::verificarRegistros('select id_usuario from usuario where correo="'.$correo.'" ');
      ...
      if($verificarCorreo > 0){
        ...
        parent::query('insert into usuario(nombre,  A_paterno,  Nombre_Usuario, correo, contrasena, confirma_contrasena) values("'.$nombre.'", "'.$A_paterno.'","'.$Nombre_Usuario.'","'.$correo.'", MD5("'.$contrasena.'"), MD5("'.$confirma_contrasena.'") )');


        echo '../proyecto1/index.php';
